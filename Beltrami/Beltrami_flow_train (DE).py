...
# PDE
# We can't sample the value like this, the present value can be considered as a continuous value, and this will lead to too many points.
# It should be taken discrete.
# x_pde = np.random.uniform(low=-1, high=1, size=(10000, 1))
# y_pde = np.random.uniform(low=-1, high=1, size=(10000, 1))
# z_pde = np.random.uniform(low=-1, high=1, size=(10000, 1))
# t_pde = np.random.uniform(low=0, high=1, size=(10000, 1))

# x_pde = np.random.randint(31, size=5500) / 15 - 1
# x_pde = x_pde.reshape(-1, 1)
# y_pde = np.random.randint(31, size=5500) / 15 - 1
# y_pde = y_pde.reshape(-1, 1)
# z_pde = np.random.randint(31, size=5500) / 15 - 1
...
for epoch in range(iterations):
    ...
    if loss < min_loss:
        min_loss = loss
        # 保存模型语句
        torch.save(net, '/home/user/ZHOU-Wen/PINN/demo/Beltrami/best_net_with_improved(DE).pth')
