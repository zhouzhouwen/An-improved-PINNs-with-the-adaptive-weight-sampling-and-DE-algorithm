...
# x_pde = qmc.Sobol(d=1, scramble=True)
# x_pde = x_pde.random(300)
# x_pde = x_pde * 1.5 - 0.5
#
# y_pde = qmc.Sobol(d=1, scramble=True)
# y_pde = y_pde.random(300)
# y_pde = y_pde * 2 - 0.5
...
# 保存模型
# torch.save(net.state_dict(), '/home/user/ZHOU-Wen/PINN/demo/Kovasznay/best_net_with_improvedt(only_para).pth')

# 保存checkpoint
# torch.save({
#     'model_state_dict': net.state_dict(),
#     'optimizer_state_dict': optimizer.state_dict(),
# }, 'checkpoint.tar'  # 这里的后缀名官方推荐使用.tar
# )
...
for i in range(resample_times):
    ...
    temp_net = torch.load('/home/user/ZHOU-Wen/PINN/demo/Kovasznay/best_net_with_improvedtt.pth')

    # x_pde = qmc.Sobol(d=1, scramble=True)
    # x_pde = x_pde.random(10000)
    # X = x_pde * 1.5 - 0.5
    #
    # y_pde = qmc.Sobol(d=1, scramble=True)
    # y_pde = y_pde.random(10000)
    # Y = y_pde * 2 - 0.5
    ...
    # pt_x_collocation = np.append(pt_x_collocation.cpu().detach().numpy(), X_selected, axis=0)
    # pt_y_collocation = np.append(pt_y_collocation.cpu().detach().numpy(), Y_selected, axis=0)

    pt_x_collocation = X_selected
    pt_y_collocation = Y_selected

    # print(pt_x_collocation)
    # print(pt_y_collocation)
    # print('xxxx')
    # print(pt_x_collocation.shape)
    # print(pt_y_collocation.shape)

    # plt.scatter(pt_x_collocation, pt_y_collocation)
    # plt.show()

    pt_x_collocation = Variable(torch.from_numpy(pt_x_collocation).float(), requires_grad=True).to(device)
    pt_y_collocation = Variable(torch.from_numpy(pt_y_collocation).float(), requires_grad=True).to(device)



    new_net = Net(6, 64)
    new_net.apply(weights_init).to(device)
    log_var_a = Variable(torch.ones(1).cuda(), requires_grad=True)
    log_var_b = Variable(torch.ones(1).cuda(), requires_grad=True)
    params = ([p for p in new_net.parameters()] + [log_var_a] + [log_var_b])
    new_optimizer = torch.optim.Adam(params)
    ...
    for epoch in range(10000):
        ...
        new_optimizer.zero_grad()  # to make the gradients zero

        # Loss based on boundary conditions
        predict = new_net(torch.cat([x_bc, y_bc], 1))  # output of u(t,x)
        u, v, p = predict[:, 0].reshape(-1, 1), predict[:, 1].reshape(-1, 1), predict[:, 2].reshape(-1, 1)
        mse_u_bc = criterion(u, u_bc, log_var_a)
        mse_v_bc = criterion(v, v_bc, log_var_a)
        mse_p_bc = criterion(p, p_bc, log_var_a)
        ...
        # Loss based on PDE
        f_u, f_v, f_e = NS_Kovasznay(pt_x_collocation, pt_y_collocation, net=new_net)  # output of f(t,x)
        mse_f1 = criterion(f_u, torch.zeros_like(f_u), log_var_b)
        mse_f2 = criterion(f_v, torch.zeros_like(f_v), log_var_b)
        mse_f3 = criterion(f_e, torch.zeros_like(f_e), log_var_b)
        ...
        loss.backward()  # This is for computing gradients using backward propagation
        new_optimizer.step()  # This is equivalent to : theta_new = theta_old - alpha * derivative of J w.r.t theta
        # scheduler.step(loss)

        if loss < min_loss:
            min_loss = loss
            # 保存模型语句
            torch.save(new_net, '/home/user/ZHOU-Wen/PINN/demo/Kovasznay/best_net_with_improvedtt.pth')
            # print('saved')
            # torch.save(new_net.state_dict(), '/home/user/ZHOU-Wen/PINN/demo/Kovasznay/best_net_with_improvedt(only_para).pth')
            # 保存checkpoint
            # torch.save({
    ...
    if i == resample_times-1:
        ...
        for epoch2 in range(1000):
            ...
            with torch.autograd.no_grad():
                if epoch2 % 100 == 0:
                    data = [mse_cost_function(f_u, torch.zeros_like(f_u)).item(),
