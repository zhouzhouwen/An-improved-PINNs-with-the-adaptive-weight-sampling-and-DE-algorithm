...
for epoch in range(iterations):
    ...
    if loss < min_loss:
        min_loss = loss
        # 保存模型语句
        torch.save(net, '/home/user/ZHOU-Wen/PINN/demo/Kovasznay/best_net_with_improved(all improvements).pth')
        print('current epoch',epoch)
    with torch.autograd.no_grad():
        if epoch % 1 == 0:
            data = [loss.item(),
                    mse_cost_function(f_u, torch.zeros_like(f_u)).item(), mse_cost_function(f_v, torch.zeros_like(f_v)).item(),
                    mse_cost_function(f_e, torch.zeros_like(f_e)).item(),
                    mse_cost_function(u, u_bc).item(), mse_cost_function(v, v_bc).item(),
                    mse_cost_function(p, p_bc).item(),log_var_a.item(),log_var_b.item(),log_var_c.item(),log_var_d.item(),log_var_e.item(),log_var_f.item()
                    ]
            with open('loss_with.csv', 'a', encoding='utf-8', newline='') as fp:
                # 写
                writer = csv.writer(fp)
